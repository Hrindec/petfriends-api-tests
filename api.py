...
class PetFriends:
        ...
        with open(pet_photo, 'rb') as photo_file:
            data = MultipartEncoder(
                fields={
                    ...
                    'age': age,
                    'pet_photo': (pet_photo, photo_file, 'image/jpeg')
                })
    ...
    def get_pet_info_by_id(self, auth_key: json, pet_id: str) -> json:
        """Метод делает запрос к API сервера и возвращает статус запроса и результат в формате JSON
        с детальной информацией о питомце по его уникальному ID"""
